...
class InvalidKafkaCluster(Exception):
    ...
    def __init__(
        self,
        environment,
        message="Environment is not a valid Kafka cluster. Use 'dev' or 'prod'",
    ):
...
class Topic:
    ...
    def _delete_blaster_topics(self, topics: dict) -> None:
        ...
        for topic_name, environment_name in topics.items():
            ...
            if response.status_code == 200:
                ...
                logging.warning(
                    f"Topic {topic_name} was not found on capability service."
                )
                ...
                logging.error(
                    f"The capability service returned status code {response.status_code}"
                )
    ...
    def _delete_confluent_topics_using_broker(topics: dict, config: dict) -> None:
        ...
        futures: dict = admin_client.delete_topics(
            list(topics.keys()), operation_timeout=30
        )
